 ...
 *	Date: 04/19/2017
 *
 * NOTE: https://community.smartthings.com/t/beta-ge-jasco-z-wave-plus-dimmer-switch-with-double-tap-associations-and-all-configuration-options/85234/72
 *
 *	Changelog:
 *
...
metadata {
	definition (name: "GE/Jasco Z-Wave Plus Dimmer Switch", namespace: "nuttytree", author: "Chris Nussbaum") {
        ...
        fingerprint mfr:"0063", prod:"4944", model:"3131", ver: "5.20", deviceJoinName: "Jasco Z-Wave Plus Toggle Fan Dimmer"
	}
...
def on() {
    ...
    cmds << zwave.basicV1.basicSet(value: 0xFF).format()
   	cmds << zwave.switchMultilevelV2.switchMultilevelGet().format()
    def delay = (device.currentValue("zwaveSteps") * device.currentValue("zwaveDelay")).longValue() * 1000 + 200
...
def off() {
    ...
    cmds << zwave.basicV1.basicSet(value: 0x00).format()
   	cmds << zwave.switchMultilevelV2.switchMultilevelGet().format()
    def delay = (device.currentValue("zwaveSteps") * device.currentValue("zwaveDelay")).longValue() * 1000 + 800
    delayBetween(cmds, delay)
}
...
def setLevel(value) {
    ...
    def delay = (device.currentValue("zwaveSteps") * device.currentValue("zwaveDelay") * level / 100).longValue() * 1000 + 300
	delayBetween ([
